...
function getStatusline() {
  ...
  try {
    ...
    if (fs.existsSync(stPath)) {
      const status = JSON.parse(fs.readFileSync(stPath, "utf8"));
      const lines = [];
      const colors = loadColors();

      if (status.input) {
        // user typed(orig) → Claude sees(trans)
        ...
        lines.push(`${c(colors.user)}${orig}${R} ${c(colors.arrow)}\u2192${R} ${c(colors.claude)}${trans}${R}`);
      }
      if (status.output) {
        // Claude said(orig) → user sees(trans)
        ...
        lines.push(`${c(colors.claude)}${orig}${R} ${c(colors.arrow)}\u2192${R} ${c(colors.user)}${trans}${R}`);
      }
      ...
      if (lines.length > 0) {
        ...
        if (status.output) {
            ...
            formatted.push(`\u21B3 ${c(colors.claude)}${orig}${R} ${c(colors.arrow)}\u2192${R} ${c(colors.user)}${trans}${R}`);
        }
