...
function initialize() {
  ...
  global.fetch = async function (input, init) {
    ...
    // --- /ts-mode slash command ---
    if (firstMsgText && firstMsgText.startsWith("/ts-mode")) {
      const val = firstMsgText.split(" ")[1]?.toLowerCase() || "";
      const cfgPath = path.join(os.homedir(), ".claude", "claude-trans.json");
      const currentCfg = loadConfig();

      if (val === "proxy" || val === "fetch") {
        currentCfg.mode = val;
        fs.writeFileSync(cfgPath, JSON.stringify(currentCfg, null, 2), "utf8");
        const label = val === "proxy" ? "Proxy (for native installs)" : "Fetch (for npm installs)";
        return new Response(JSON.stringify({
          type: "message", role: "assistant",
          content: [{ type: "text", text: `[claude-trans] Mode set to ${label}. Restart cts to apply.` }]
        }), { status: 200, headers: { "content-type": "application/json" } });
      } else if (!val) {
        return new Response(JSON.stringify({
          type: "message", role: "assistant",
          content: [{ type: "text", text: `[claude-trans] Current mode: ${currentCfg.mode || "fetch"}\nUsage: /ts-mode proxy | /ts-mode fetch` }]
        }), { status: 200, headers: { "content-type": "application/json" } });
      }
    }
