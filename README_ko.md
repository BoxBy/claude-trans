...
> **참고:** 기본 모드는 npm으로 설치한 Claude Code가 필요합니다. 네이티브 인스톨러(`curl -fsSL https://claude.ai/install.sh | bash`)를 사용한 경우 `--use-proxy` 플래그를 사용하세요.
...
Claude Code를 **네이티브 인스톨러**(`curl -fsSL https://claude.ai/install.sh | bash`)로 설치한 경우 `--use-proxy` 플래그를 사용하세요:
